...
export const updateThought = async (req, res) => {
    ...
    try {
        const updatedThought = await Thought.findByIdAndUpdate(thoughtId, newThought, { new: true });
        if (!updatedThought) {
...
export const deleteThought = async (req, res) => {
    ...
    try {
        ...
        await User.findByIdAndUpdate({ thoughts: thought._id }, { $pull: { thoughts: thought._id } });
...
export const removeReaction = async (req, res) => {
    ...
    try {
        const thought = await Thought.findByIdAndUpdate(thoughtId, { $pull: { reactions: { reactionId: reactionId } } }, { new: true });
        if (!thought) {
            return res.status(404).json({ message: 'Thought does not exist.' });
        }
        return res.json(thought);
